...
class MateRunnerTest {
    ...
    private class Reducer : MateReducer<S, Msg, Effect> {
        override fun reduce(state: S, message: Msg): ReducerResult<S, Effect> =
            when (message) {
                is Msg.Add -> state.copy(value = state.value + message.amount)
    ...
    private class EchoHandler : MateTypedEffectHandler<Msg, Fx.Echo>() {
        ...
        override suspend fun onEffect(effect: Fx.Echo, consumer: (Msg) -> Unit) {
            consumer(Msg.Echoed(effect.tag))
        }
    }

    @Test
    fun acceptReducesAndPublishesState() = runTest {
        val mate = Mate<S, Msg, Effect>(
            initState = S(0),
            reducer = Reducer(),
    ...
    @Test
    fun initEffectsRunOnStart() = runTest {
        val mate = Mate<S, Msg, Effect>(
            initState = S(0),
            reducer = Reducer(),
    ...
    @Test
    fun flowHandlerSubscribesOnStartAndUnsubscribesOnDispose() = runTest {
        val source = MutableSharedFlow<String>()
        val flowHandler = object : MateFlowHandler<Msg, Effect> {
            ...
            override suspend fun runEffect(effect: Effect, consumer: (Msg) -> Unit) = Unit
        }
        val mate = Mate<S, Msg, Effect>(
            initState = S(0),
            reducer = Reducer(),
